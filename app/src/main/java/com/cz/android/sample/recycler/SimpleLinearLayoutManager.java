...
public class SimpleLinearLayoutManager extends RecyclerView.LayoutManager {
     ...
     * helps {@link SimpleLinearLayoutManager} make those decisions.
    ...
    /**
     * Sets the orientation of the layout. {@link SimpleLinearLayoutManager}
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (legacyStackFromEndChanged == false && getChildCount() > 0) {
            ...
            mRenderState.mCurrentPosition = mStackFromEnd ? state.getItemCount() - 1 : 0;
        }
        ...
        fill(recycler, state, mRenderState, false);
//        removeAndRecycleScrap(recycler);
    ...
    private int scrollBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        final int consumed = freeScroll + fill(recycler, state, mRenderState, false);
        if (consumed < 0) {
            if (DEBUG) {
     ...
     * @see SimpleLinearLayoutManager.RenderState#mLayoutDirection
     */
     ...
     *
     * @param recycler             Current recycler that is attached to RecyclerView
     * @param state              Current adapter that is attached to RecyclerView
    ...
    private int fill(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        while (renderState.mAvailable > 0 && renderState.hasMore(state)) {
            View view = renderState.next(recycler, state);
            if (mShouldReverseLayout) {
                if (renderState.mLayoutDirection == RenderState.LAYOUT_START) {
                    addView(view);
                } else {
    ...
    private static class RenderState {
        final static String TAG = "LinearLayoutManager";
         ...
         * {@link #fill(RecyclerView.Recycler, RecyclerView.State, RenderState, boolean)} tries to
         * fill
