 ...
 *
 * <p>
 * Also see {@link LayoutParams android.widget.LinearLayout.LayoutParams}
 * for layout attributes </p>
 */
public class SimpleLinearLayout extends SimpleViewGroup {
    ...
    public SimpleLinearLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = 
            context.obtainStyledAttributes(attrs, R.styleable.SimpleLinearLayout);
        int index = a.getInt(R.styleable.SimpleLinearLayout_simple_orientation, -1);
        if (index >= 0) {
            setOrientation(index);
        }
        index = a.getInt(R.styleable.SimpleLinearLayout_simple_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }
        boolean baselineAligned = a.getBoolean(R.styleable.SimpleLinearLayout_simple_baselineAligned, true);
        if (!baselineAligned) {
            setBaselineAligned(baselineAligned);
        }
        mWeightSum = a.getFloat(R.styleable.SimpleLinearLayout_simple_weightSum, -1.0f);
        mBaselineAlignedChildIndex =
                a.getInt(R.styleable.SimpleLinearLayout_simple_baselineAlignedChildIndex, -1);
        // TODO: Better name, add Java APIs, make it public
        mUseLargestChild = a.getBoolean(R.styleable.SimpleLinearLayout_simple_useLargestChild, false);
        a.recycle();
    }
    /**
    ...
    public int getBaseline() {
        ...
        if (mOrientation == VERTICAL) {
            ...
            if (majorGravity != Gravity.TOP) {
                ...
                switch (majorGravity) {
                   case Gravity.BOTTOM:
                       childTop = bottom - top - paddingBottom - mTotalLength;
                       break;
                   case Gravity.CENTER_VERTICAL:
                       childTop += ((bottom - top - paddingTop - paddingBottom) -
                               mTotalLength) / 2;
                       break;
               }
            }
        ...
        if (majorGravity != Gravity.TOP) {
           switch (majorGravity) {
               case Gravity.BOTTOM:
                   ...
                   childTop = bottom - top + paddingTop - mTotalLength;
                   break;
               case Gravity.CENTER_VERTICAL:
                   childTop += ((bottom - top)  - mTotalLength) / 2;
                   break;
           }
        ...
        if (majorGravity != Gravity.LEFT) {
            switch (majorGravity) {
                case Gravity.RIGHT:
                    ...
                    // padding to compensate
                    childLeft = right - left + paddingLeft - mTotalLength;
                    break;
                case Gravity.CENTER_HORIZONTAL:
                    childLeft += ((right - left) - mTotalLength) / 2;
                    break;
            }
       }
        for (int i = 0; i < count; i++) {
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            weight = a.getFloat(R.styleable.LinearLayout_Layout_simple_layout_weight, 0);
            gravity = a.getInt(R.styleable.LinearLayout_Layout_simple_layout_gravity, -1);
            a.recycle();
        }
        /**
         * {@inheritDoc}
