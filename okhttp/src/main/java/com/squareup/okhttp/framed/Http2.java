...
public final class Http2 implements Variant {
  ...
  static final class Reader implements FrameReader {
    ...
    @Override public boolean nextFrame(Handler handler) throws IOException {
      ...
      if (logger.isLoggable(FINE)) logger.fine(FrameLogger.formatHeader(true, streamId, length, type, flags));
  ...
  static final class Writer implements FrameWriter {
    ...
    void frameHeader(int streamId, int length, byte type, byte flags) throws IOException {
      if (logger.isLoggable(FINE)) logger.fine(FrameLogger.formatHeader(false, streamId, length, type, flags));
