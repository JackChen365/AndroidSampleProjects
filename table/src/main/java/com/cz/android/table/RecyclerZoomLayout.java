...
public abstract class RecyclerZoomLayout extends ZoomLayout {
    ...
    /**
     * Measure a table cell by its layout params.
     * @param view
     */
    protected void measureChildView(View view){
        ViewGroup.LayoutParams layoutParams = view.getLayoutParams();
        measureChildView(view,layoutParams.width,layoutParams.height);
    }

    /**
     * For a table cell, Even though we it might have its own dimension,
     * but we actually have to use the exactly size of the table cell.
     * @param view
     * @param childWidth
     * @param childHeight
     */
    protected void measureChildView(View view,int childWidth,int childHeight){
        ...
        int childHeightMeasureSpec = getChildMeasureSpec(parentHeightMeasureSpec, 0, childHeight);
    ...
    /**
     * We won't measure the layout by its measurement mode. Since we are a table or something like a table.
     * I guess you won't use a table to load a few cells, usually the table will fill its parent's content.
     *
     * Keep the measure specification in order to measure the ChildView correctly.
